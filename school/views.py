...
def check_password_pattern(plain_password):
    pattern_password = "^(?=\P{Ll}*\p{Ll})(?=\P{Lu}*\p{Lu})(?=\P{N}*\p{N})(?=[\p{L}\p{N}]*[^\p{L}\p{N}])[\S]{8,}$"

    # compiling pattern_password regex
    pattern = regex.compile(pattern_password)
    # searching pattern in password
    pattern_match = regex.search(pattern, plain_password)
    if pattern_match:
        return True
    return False
...
def student_signup_view(request):
    ...
    if request.method=='POST':
        ...
        #print(password)
        pass_word= check_password_pattern(password)
        #print(pass_word)
        if pass_word == False:
            messages.error(request, 'Please write strong password.')
